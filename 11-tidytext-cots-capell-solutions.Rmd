---
title: "11-tidytext - exercises with solutions"
...
<br>
*Note*: this is the file with answers to the exercises. 
...
````{r}
#Usage of word "I"
times_I <- together_words %>% #we have to use initial dataframe with stopwords (bc I is considered a stopword)
  group_by(president) %>% 
  count(word == "i") %>% 
  mutate(share_I = 100*(n/sum(n))) %>%
  select(!n) %>% 
  pivot_wider(
    names_from = president, 
    values_from = share_I, 
...
#Usage of word "we"
times_we <- together_words %>% #use initial dataframe with stopwords (bc we is considered a stopword)
  group_by(president) %>% 
  count(word == "we") %>% 
  mutate(share_we = 100*(n/sum(n))) %>%  #the numbers we get are already readable as percentages
  select(!n) %>% 
  pivot_wider(
    names_from = president, 
...
I_we_share <- I_we_share[c(2,4),] #we only want the rows for "TRUE" values, which means for when words "I" or "we" appeared in the text. This happens in rows 2 and 4.

#We add a column to know what word it is
word_used <- c("I", "we")
word_type <- data.frame(word_used)
I_we_share %>% 
  bind_cols(word_type) 
